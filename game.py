...
class Game:
    ...
    def __get_map(self):
        ...
        self.board[snake[:, 1], snake[:, 0]] = 2/3  # body = 1
        if self.running:
            snake = self.snake.snake[0]
            self.board[snake[1]][snake[0]] = 3/3  # head = 1
        self.board[:, 0] = 2/3  # left wall
        self.board[:, -1] = 2/3  # right wall
        self.board[0, :] = 2/3  # top wall
        self.board[-1, :] = 2/3  # bottom wall
        self.board[self.food[1]][self.food[0]] = 1/3  # food = 0.5
        return self.board.reshape((self.width + 2, self.height + 2))
